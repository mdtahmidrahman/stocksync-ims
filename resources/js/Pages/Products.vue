  ...
  <AppLayout>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      ...
      <div class="flex items-center gap-3 self-start sm:self-auto">
        <button @click="showSyncModal = true" class="bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 border border-gray-300 dark:border-gray-700 transition-colors shadow-sm flex items-center gap-2">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" v-for="product in localProducts" :key="product.id">
              <td class="p-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded bg-gray-100 dark:bg-gray-700 flex items-center justify-center shrink-0 overflow-hidden">
                    <img v-if="product.image_path" :src="product.image_path.startsWith('http') ? product.image_path : `/storage/${product.image_path}`" class="w-full h-full object-cover" />
              ...
              <td class="p-4">
                ...
                <span v-else class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ product.stock_quantity }}</span>
              </td>
              <td class="p-4 text-right whitespace-nowrap">
                <div class="flex items-center justify-end gap-2">
                  <button @click="openHistory(product)" class="text-primary-600 dark:text-primary-400 hover:text-primary-900 dark:hover:text-primary-300 text-sm font-medium mr-2">History</button>
    ...
    <Modal :show="showSyncModal" @close="showSyncModal = false" title="Stock Synchronization / Adjustment">
      <template #body>
        <form @submit.prevent="submitSync" id="syncStockForm" class="space-y-4">
          <div>
            ...
            <select v-model="syncForm.product_id" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-black text-gray-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
              ...
              <option v-for="prod in props.products.data" :key="prod.id" :value="prod.id">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select v-model="syncForm.warehouse_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-black text-gray-900 dark:text-white focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                ...
                <option v-for="wh in props.warehouses" :key="wh.id" :value="wh.id">{{ wh.name }}</option>
              </select>
            </div>
          </div>
          <div>
      ...
      <template #footer>
        ...
        <button type="submit" form="syncStockForm" :disabled="syncForm.processing" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-colors">
...
const openEditModal = (product) => {
    ...
    editForm.image = null;
    editImagePreview.value = product.image_path ? (product.image_path.startsWith('http') ? product.image_path : `/storage/${product.image_path}`) : null;
...
const openHistory = (product) => {
  ...
  fetch(`/products/history/${product.id}`)
    .then(res => res.json())
    .then(data => {
